...
本节叙述一些在初等数学领域非常重要的不等式\footnote{这一节主要参考了文献\cite{contest-math-course}和\cite{calculus-course}以及\cite{olympic-math}.}，这些不等式应用广泛，刻画的不等式关系非常深刻。
...
我们在\autoref{sec:polynomial-power-theorem}中曾经利用多项式乘幂定理证明过中间的$A_n \geqslant G_n$，
在\autoref{thm:mathematical-induction-3}中也曾经使用倒推形式的数学归纳法证明过这个不等式，所以在这一节我们使用另外的方法。
...
以下推论在不等式证明中经常会用到
\begin{inference}
  设 $a_{i}>0(i=1,2,\ldots,n)$和$b_i>0(i=1,2,\ldots,n)$，有
  \[ \frac{a_1^2}{b_1}+\frac{a_2^2}{b_2}+\cdots+\frac{a_n^2}{b_n} \geqslant \frac{(a_1+a_2+\cdots+a_n)^{2 }}{b_{1}+b_2+\cdots+b_n} \]
  取等条件仍然是 $a_i=\lambda b_i(i=1,2,\ldots,n)$, 这里 $\lambda >0$ 为常数.
\end{inference}
其证明很简单，把右边的分母乘到左边，再构造两组数 $\frac{a_i}{\sqrt{b_i}}$和$\sqrt{b_i}$应用柯西不等式即可.

\begin{example}
  假设 $x,y,z \geqslant 1$，且
  \[ \frac{1}{x} + \frac{1}{y} + \frac{1}{z} = 2 \]
  证明
  \[ \sqrt{x+y+z} \geqslant \sqrt{x-1} + \sqrt{y-1} + \sqrt{z-1} \]
  \begin{proof}[证明]
    记 $a=\sqrt{x-1}$, $b=\sqrt{y-1}$, $c=\sqrt{z-1}$，则条件中的等式即为
    \[ \frac{1}{1+a^2} + \frac{1}{1+b^2} + \frac{1}{1+c^2} = 2 \]
    要证明的不等式即为
    \[ 3+a^2+b^2+c^2 \geqslant (a+b+c)^2 \]
    由柯西不等式的推论，有
    \[ \frac{a^2}{1+a^2} + \frac{b^2}{1+b^2} + \frac{c^2}{1+c^2} \geqslant \frac{(a+b+c)^{2}}{3+a^2+b^2+c^2} \]
    另一方面有
    \[ \frac{a^2}{1+a^2} + \frac{b^2}{1+b^2} + \frac{c^2}{1+c^2} = 3-\left( \frac{1}{1+a^2} + \frac{1}{1+b^2} + \frac{1}{1+c^2} \right) = 1 \]
    因此得证.
  \end{proof}
\end{example}
...
\begin{proof}[证明]
  ...
  至于不等式的左边，只要将序列$y_1,y_2,\ldots,y_n$替换为序列$-y_n,-y_{n-1},\ldots,-y_1$后应用不等式的右边即可得出(当然也可以使用排序不等式的左边也像刚才那样累加得出)。
\end{proof}
...
先考虑两个$n$维向量，$\bm{x}=(x_1,x_2,\ldots,x_n)$和$\bm{y}=(y_1,y_2,\ldots,y_n)$，把关于二维和向量的不等式推广有
\[ |\bm{x+y}| \leqslant |\bm{x}| + |\bm{y}| \]
写成坐标形式就是
\begin{theorem}[闵可夫斯基不等式]
  设 $x_i(i=1,2,\ldots,n)$ 和 $y_i(i=1,2,\dots,n)$是两组实数，有
\[ \sqrt{\sum_{i=1}^n(x_i+y_i)^2} \leqslant \sqrt{\sum_{i=1}^nx_i^2} + \sqrt{\sum_{i=1}^ny_i^2} \]
等号成立的条件是存在正实数$\lambda>0$，使得$ \frac{x_i}{y_i} = \lambda $ 对 $i=1,2,\ldots,n$ 同时成立(约定分母为零时，分子也为零).
\end{theorem}

\begin{proof}[证明]
  采用数学归纳法，$n=2$时不等式即为
  \[ \sqrt{(x_1+y_1)^2+(x_2+y_2)^2} \leqslant \sqrt{(x_1^2+x_2^2)} + \sqrt{(y_1^2+y_2^2)} \]
  两边平方后容易验证它是正确的.

  假若不等式对$n$的情形已经成立，那么对于 $n+1$ 的情形，左边的平方即是
  \begin{equation*}
    \begin{split}
      \sum_{i=1}^{n+1}(x_{i}+y_{i})^2  & =   \sum_{i=1}^{n}(x_i+y_i)^2 + (x_{n+1}+y_{n+1})^2 \\
      & \leqslant  \left( \sqrt{\sum_{i=1}^nx_i^2} + \sqrt{\sum_{i=1}^ny_i^2} \right)^2 + (x_{n+1}+y_{n+1})^2 \\
      & =  \sum_{i=1}^{n+1}x_i^2 + \sum_{i=1}^{n+1}y_i^2 + 2 \sqrt{\sum_{i=1}^nx_i^2\sum_{i=1}^ny_i^2} + 2 x_{n+1}y_{n+1}
    \end{split}
  \end{equation*}
  而右边的平方则是
  \[ \sum_{i=1}^{n+1}x_i^2 + \sum_{i=1}^{n+1}y_i^2 + 2 \sqrt{\sum_{i=1}^{n+1}x_i^2\sum_{i=1}^{n+1}y_i^2} \]
  因此只需要证明
  \[ \sqrt{\sum_{i=1}^nx_i^2\sum_{i=1}^ny_i^2} +  x_{n+1}y_{n+1} \leqslant \sqrt{\sum_{i=1}^{n+1}x_i^2\sum_{i=1}^{n+1}y_i^2} \]
  两边平方，即是要证
  \[ 2x_{n+1}y_{n+1}\sqrt{\sum_{i=1}^nx_i^2\sum_{i=1}^ny_i^2} \leqslant x_{n+1}^{2}\sum_{i=1}^ny_i^2 + y_{n+1}^2\sum_{i=1}^nx_i^2 \]
  这显然成立，因此不等式对于$n+1$的情形也成立.
\end{proof}

同样利用数学归纳法，容易推广到 $m$ 组数的形式(参考$m$个$n$维向量之和的模与各个向量的模之和的大小关系):
\[ \sqrt{\sum_{j=1}^n(\sum_{i=1}^mx_{ij})^2} \leqslant \sum_{i=1}^m \sqrt{\sum_{j=1}^nx_{ij}^2}  \]
...
\exerciseFrom[2017年德国数学奥林匹克试题\\
\url{http://kuing.orzweb.net/viewthread.php?tid=4578}]
