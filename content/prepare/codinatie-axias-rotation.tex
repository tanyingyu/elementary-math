...
这些变换包括\emph{平移}、\emph{旋转}、\emph{伸缩}、\emph{对称}，通常所说的位似变换可以通过伸缩变换得到，
其中，平移、旋转、对称都是恒等变换，即不改变平面图形的形状和大小，
或者说变换前后的图形是全等的，而位似变换前后的图象则是相似的，形状不变，大小可以改变。
...
\begin{forest}
  ...
  for tree={
    ...
    thick,     % 边框粗细
    % 连接线样式
    edge={-stealth, thick, gray!70}, % 使用箭头和灰色粗线
    parent anchor=south,
    child anchor=north,
    % 树的布局间距
    l sep=1cm, % 层级间距 (垂直)
    s sep=0.3cm, % 同级节点间距 (水平)
    % 根据层级设置颜色
    % Level 1: 主类别
    where level=1{fill=teal!30, edge=teal!70!black}{},
    % Level 2: 子类别
    where level=2{fill=blue!15 }{},
    % Level 3: 孙类别
    where level=3{fill=gray!10}{},
  }
  [坐标变换[恒等变换[平移][旋转][对称]] [位似变换[伸缩]]]
\end{forest}


\subsection{平移}

平移变换由平移向量$\vv{s}=(a,b)$唯一确定，平面上任意一点$P$与它在变换下的像$P'$满足$\vv{PP'}=\vv{s}$为常向量，
因此如果点$P$的坐标是$(x,y)$，点$P'$的坐标$(x',y')$将是$(x+a, y+b)$，即
\begin{equation}
...
\begin{example}
  ...
  \begin{equation*}
    \begin{split}
      & \cos{\theta}(\sin{\theta}-a\cos{\theta})x^2 -  \sin{\theta}(\cos{\theta}+a\sin{\theta})y^2 \\
...
\begin{equation}
  \label{eq:scale-translation-coordinate-formula-1}
  \left\{
    \begin{array}{ccc}
      x' & = & \lambda x \\
      y' & = & \mu y
...
\begin{equation}
  \label{eq:scale-translation-coordinate-formula-2}
  \left\{
    \begin{array}{ccc}
      x' & = &  x + uy \\
      y' & = &  y
...
\begin{equation}
  \label{eq:scale-translation-coordinate-formula-3}
  \left\{
    \begin{array}{ccc}
      x' & = &  x \\
      y' & = &  vx+y
